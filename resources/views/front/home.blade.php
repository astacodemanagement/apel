...
@section('content')
    ...
    <div class="offers md:pt-10 text-md">
        <p class="text-sm">et up to ₹8000 instant savings on selected products with eligible HDFC Bank cards. Plus No Cost
            EMI from most leading banks.
        </p>
    ...
    <div class="products py-10">
        ...
        <div class="container flex md:justify-between justify-center md:gap-0 w-full flex-wrap items-center gap-3">
            @foreach ($kategori_produk as $p)
                <div
                    class="product md:w-[12%] w-[45%] bg-gray-100 border-2 border-gray-200 my-2 items-center p-4 rounded-lg  hover:scale-105 transition duration-300">
        ...
        <div class="grid grid-cols-12 gap-4">
            @foreach ($galeri as $p)
                <div class="col-span-6 flex justify-center relative group">
                    <img src="/upload/galeri/{{ $p->gambar }}"
                        alt="events"
                    ...
                    <div
                        ...
                        <h2
                            class="text-white text-xl font-semibold mb-2 group-hover:scale-105 transform transition-transform duration-300">
                            {{ $p->nama_galeri }}</h2>
                        <a href="{{ $p->link }}"
                            class="px-4 py-2 bg-white text-black rounded-lg font-medium hover:bg-gray-200 group-hover:scale-105 transform transition-transform duration-300">Learn
                            More</a>
                    </div>
                </div>
            @endforeach
